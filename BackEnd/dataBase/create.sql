...
(12) not null,
  primary key
(`post_id`, `surfer_id`),
  key `fk_fav_post_surfer`
(`surfer_id`),
  constraint `fk_fav_post_post` foreign key
(`post_id`) references `post`
(`id`) on
delete cascade,
  constraint `fk_fav_post_surfer` foreign key
(`surfer_id`) references `surfer`
(`id`) on
delete cascade
);
...
BEGIN
	...
	from fav_post
	JOIN post
	  ON fav_post.post_id = post.id
	JOIN surfer
	  ON surfer.id = post.surfer_id
	LEFT JOIN `like`
	  ON `like`.post_id = post.id
	where fav_post.surfer_id = userId
    group by post.id;
END
